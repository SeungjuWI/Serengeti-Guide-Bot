...
| `src/pinned.js` | 특정 주제 → 지정 문서 고정 (경조사 등) |
...
공식 규정이 한 페이지에만 있는 주제는 그 문서에서만 답하도록 고정할 수 있습니다.
고정된 주제는 검색 후보를 지정 페이지로만 좁히고, 다른 문서에 비슷한 내용이 있어도 근거로 쓰지 않습니다.

| 주제 | 고정 문서 |
|---|---|
| 경조사 (경조금·경조휴가·화환·상조물품, 결혼/사망/칠순) | [경조사 지원 제도](https://app.notion.com/p/946b4edc8c344aef8193eb5e01a09ec6) |

문서만으로는 모델이 자꾸 틀리는 지점(예: 표의 `(본인)` 표기를 넘겨짚어 배우자 쪽까지 지원된다고 답하는 것)은
`notes`에 해석 기준을 적어두면 답변 생성 시 문서 본문보다 우선해 적용됩니다. **규정이 바뀌면 노션 문서와 `notes`를 함께 고쳐야 합니다.**

규칙은 `src/pinned.js`에 있습니다. 어떤 표현이 이 주제로 잡히는지(`patterns`)와 고정 문서(`pageIds`), 해석 기준(`notes`)을 여기서 조정하세요.
문서를 옮겨서 페이지 ID만 바꾸면 될 때는 `.env`의 `PINNED_TOPICS=경조사:페이지아이디` 로도 덮어쓸 수 있습니다.
고정 문서가 아직 인덱스에 없으면 노션에서 그 페이지를 직접 읽어 답합니다.
