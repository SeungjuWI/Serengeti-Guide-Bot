    ...
    messages: [
      {
        ...
        content:
          ...
          "(인사규정, 휴가, 복지, 급여일, 근무환경, 사무기기, 비품, 구매·계약, 회의실/장비 예약, 조직, 구성원, 사내 행사, 그룹웨어 사용법, " +
          "전자결재·지출·증빙·세무 처리 등)와 " +
          ...
          '- 세무·회계 용어(사업소득, 기타소득, 원천징수, 세금계산서, 지출결의서, 기부금 영수증, 필요경비, 세율 등)는 사내 지출·증빙 처리와 직결되므로 relevant=true다. 일반 상식 질문으로 보지 마라.\n' +
          ...
          '- 문의처를 묻는 질문("어디에 물어봐야 하나요?", "이건 누구한테 문의해요?", "가이드에 없는 건 어떻게 해요?")도 relevant=true다. 담당팀을 안내해야 하기 때문이다.\n' +
          ...
          '- 사내 맥락으로 좁혀서 키워드를 뽑아라. 회사에서 "카드"는 법인카드를, "결재"는 전자결재를, "한도"는 법인카드 한도를 뜻한다 (예: "카드 한도" → "법인카드 한도", "카드 써도 돼?" → "법인카드").\n' +
...
- 제공된 문서가 1건뿐이어도, 그 안에 답이 있으면 그것만으로 충분히 답한다. 문서 개수는 답변 가능 여부의 판단 기준이 아니다.
  검색이 관련도가 낮은 문서를 걸러내고 정확한 문서 하나만 넘겨주는 경우가 많다. 문서가 적다는 이유로 "찾지 못했어요"라고 답하지 않는다.
- 질문의 표현이 문서의 용어와 정확히 일치하지 않아도, 같은 것을 가리키는 내용이 문서에 있으면 그것을 근거로 답한다
  (예: "법인카드 한도" → 문서의 "카드 자체구매 … 한도/승인: 500만원 미만", "구매" → "전사 구매, 계약 프로세스").
  단어가 그대로 안 나온다는 이유로 "찾지 못했어요"라고 답하지 않는다.
- 질문의 일부만 문서로 답할 수 있으면, 답할 수 있는 만큼 먼저 답하고 나머지에 대해서만 담당팀 문의를 안내한다. 전부 거절하지 않는다.
- 질문이 특정 사내 문서·제도를 지목하고 그 문서가 검색 결과에 있으면, 그 문서의 핵심 내용을 안내한다.
  질문이 구어체이거나 띄어쓰기·오타로 뭉개져 있어도 지목된 문서를 찾아준다 (예: "구성원알아가기검색하면안돼?" → 구성원 소개 문서를 안내).
...
- 질문 주제가 모호해 담당팀을 하나로 좁힐 수 없으면 팀 목록을 안내하되, 그때는 HR팀·GA팀·FA팀 셋을 반드시 모두 적는다. 일부만 적고 FA팀을 빠뜨리지 않는다:
  - 인사규정/복리후생 관련 문의: ${describeTeam("hr")}
  - 근무환경/계약/자산관리 관련 문의: ${describeTeam("ga")}
  - 재무회계 가이드 관련 문의: ${describeTeam("fa")}
